...
import static com.pharos.walker.utils.DeviceConnFactoryManager.CONN_METHOD.USB;
...
public class DeviceConnFactoryManager {
    ...
    private static final int tip=0x010;
    private static final int abnormal_Disconnection=0x011;//异常断开
        ...
        public void handleMessage(Message msg) {
            switch (msg.what) {
                ...
                case DEFAUIT_COMMAND://默认模式
                    ToastUtils.showShort("默认模式，小票模式");
                    break;
                case READ_DATA:
...
//                            ToastUtils.showShort("票据模式");
    ...
    private void sendStateBroadcast(int state) {
        ...
        intent.putExtra(DEVICE_ID, id);
        MyApplication.getInstance().sendBroadcast(intent);//此处若报空指针错误，需要在清单文件application标签里注册此类，参考demo
    }
